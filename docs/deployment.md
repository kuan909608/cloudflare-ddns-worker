...
1. 本機先驗證 migration；在 Worker Dashboard 設定正式 `APP_HOST`、固定且唯一的 `DNS_ZONE_ID`、`DNS_ZONE_NAME` 等 runtime variables，再執行首次 deploy。`wrangler.jsonc` 不宣告 production `vars` 且使用 `keep_vars:true`，避免後續 Git 部署覆蓋 Dashboard 現值。Wrangler 會從 draft `DDNS_DB` binding 自動建立並綁定唯一 D1。此時不要關聯公開網域。
