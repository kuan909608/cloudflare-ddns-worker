import type { Client, ClientInput, UpdateLog } from '../types';
...
export const adminApi = {
  ...
  remove:(id:string)=>api<{deleted:boolean}>(`/api/admin/clients/${encodeURIComponent(id)}`,{method:'DELETE'}),
  action:(id:string,action:'enable'|'disable')=>api<Client>(`/api/admin/clients/${encodeURIComponent(id)}/${action}`,{method:'POST'}),
  rotate:(id:string)=>api<{client:Client;token:string}>(`/api/admin/clients/${encodeURIComponent(id)}/rotate-token`,{method:'POST'}),
