...
function repository():ClientRepository{return{list:vi.fn(),findById:vi.fn(async()=>client),findBySlug:vi.fn(async()=>client),create:vi.fn(),update:vi.fn(),claimRecordProvisioning:vi.fn(async()=>true),bindProvisionedRecord:vi.fn(async()=>client),releaseRecordProvisioning:vi.fn(),setEnabled:vi.fn(),rotateToken:vi.fn(),updateStatus:vi.fn(async(_id,value)=>{status=value;}),remove:vi.fn(),addLog:vi.fn(async log=>{logs.push(log);}),logs:vi.fn(),audit:vi.fn(),dashboard:vi.fn()};}
function dns(content='1.1.1.1'){const record:DnsRecord={id:'record',zoneId:'zone',zoneName:'example.com',name:'home.example.com',type:'A',content,ttl:1};return{getZone:vi.fn(async()=>({id:'zone',name:'example.com'})),getRecord:vi.fn(async()=>record),findRecords:vi.fn(async()=>[]),create:vi.fn(async(_zoneId,_name,_type,newContent)=>({...record,content:newContent})),update:vi.fn(async()=>undefined)} as DnsRecordGateway;}
...
describe('DDNS update use case',()=>{
  ...
  it('creates and permanently binds a pending Record from the first authenticated source IP',async()=>{
    ...
    expect(api.findRecords).toHaveBeenCalledWith('zone','home.example.com','A');
    expect(api.create).toHaveBeenCalledWith('zone','home.example.com','A','8.8.8.8');
  ...
  it('recovers an unbound Record left by an interrupted first update without creating a duplicate',async()=>{
    ...
    vi.mocked(api.findRecords).mockResolvedValue([await api.getRecord('zone','record')]);
