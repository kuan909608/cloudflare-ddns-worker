...
describe('source IP validation',()=>{
  ...
  it('uses only a valid CF-Connecting-IP and never falls back to XFF',()=>{let request=new Request('https://x',{headers:{'CF-Connecting-IP':'1.1.1.1','X-Forwarded-For':'8.8.8.8, 9.9.9.9'}});expect(sourceIp(request,'A')).toBe('1.1.1.1');request=new Request('https://x',{headers:{'CF-Connecting-IP':'invalid','X-Forwarded-For':'10.0.0.1, 8.8.4.4'}});expect(sourceIp(request,'A')).toBeNull();expect(sourceIp(new Request('https://x',{headers:{'X-Forwarded-For':'8.8.4.4'}}),'A')).toBeNull();});
  ...
  it('uses only a valid Cloudflare connecting IP for the pre-auth rate key',()=>{expect(rateLimitSource(new Request('https://x',{headers:{'CF-Connecting-IP':'8.8.8.8','X-Forwarded-For':'9.9.9.9'}}))).toBe('8.8.8.8');expect(rateLimitSource(new Request('https://x',{headers:{'X-Forwarded-For':'9.9.9.9'}}))).toBeNull();expect(rateLimitSource(new Request('https://x',{headers:{'CF-Connecting-IP':'invalid'}}))).toBeNull();});
});
